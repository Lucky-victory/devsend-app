...
import { Plus, Search, Filter, Upload, Download, MoreHorizontal, Mail, User, Tag } from "lucide-react"
...
export default function ContactsPage() {
  ...
  const contacts = useQuery(api.contacts.getContacts, currentWorkspace ? { workspaceId: currentWorkspace._id } : "skip")
  ...
  return (
    <motion.div className="space-y-8" initial="initial" animate="animate" variants={staggerContainer}>
      ...
      <motion.div className="grid gap-4 md:grid-cols-4" variants={staggerContainer}>
        {[
          { title: "Total Contacts", value: contactStats.total.toLocaleString(), change: "+12%", icon: User },
          { title: "Subscribed", value: contactStats.subscribed.toLocaleString(), change: "+8%", icon: Mail },
          { title: "Unsubscribed", value: contactStats.unsubscribed.toLocaleString(), change: "+2%", icon: User },
          { title: "Segments", value: contactStats.segments.toString(), change: "+3", icon: Tag },
        ].map((stat, index) => (
      ...
      <motion.div variants={fadeInUp}>
        <Card className="glassmorphism">
          <CardHeader>
            ...
            <CardDescription>Manage your email subscribers and their information</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {filteredContacts.map((contact) => (
                  <TableRow key={contact._id}>
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-3">
                        <Avatar className="h-8 w-8">
                          <AvatarImage src="/placeholder.svg" alt={contact.firstName || contact.email} />
                          <AvatarFallback>
                    ...
                    <TableCell className="text-sm">{new Date(contact.subscribedAt).toLocaleDateString()}</TableCell>
                    <TableCell className="text-sm">
                      {contact.lastActivityAt ? new Date(contact.lastActivityAt).toLocaleDateString() : "Never"}
                    </TableCell>
                    <TableCell>
                      <DropdownMenu>
      ...
      {/* Import Dialog */}
      <ImportDialog open={isImportDialogOpen} onOpenChange={setIsImportDialogOpen} workspaceId={currentWorkspace._id} />
    </motion.div>
  )
}
